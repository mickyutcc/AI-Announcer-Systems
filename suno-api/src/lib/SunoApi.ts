...
import { BrowserContext, Page, Locator, firefox, chromium } from 'rebrowser-playwright-core';
...
class SunoApi {
  ...
  */
  ...
  public async getCaptcha(): Promise<string|null> {
    if (yn(process.env.SKIP_CAPTCHA, { default: false })) 
      return null;
    if (!await this.captchaRequired())
      return null;
    if (!process.env.TWOCAPTCHA_KEY)
      throw new Error('TWOCAPTCHA_KEY is required for CAPTCHA solving');
    const currentKey = (process.env.TWOCAPTCHA_KEY || '') + '';
    if (!this.solver || this.solverKey !== currentKey) {
      this.solver = new Solver(currentKey);
      this.solverKey = currentKey;
    }
    ...
    try {
      ...
      logger.error('Timeout waiting for project API. Current URL: ' + page.url());
      throw e;
    }
    ...
    try {
      await visibleTextarea.waitFor({ state: 'visible', timeout: 10000 });
      await this.click(visibleTextarea);
      await visibleTextarea.pressSequentially('Lorem ipsum', { delay: 80 });
    } catch (e) {
      try {
        await fallbackTextarea.waitFor({ state: 'attached', timeout: 30000 });
        await fallbackTextarea.evaluate((el) => {
          const textareaEl = el as HTMLTextAreaElement;
          textareaEl.value = 'Lorem ipsum';
          textareaEl.dispatchEvent(new Event('input', { bubbles: true }));
        });
      } catch (innerError) {
        logger.error('Failed to find textarea. Current URL: ' + page.url());
        const content = await page.content();
        logger.error('Page content: ' + content.substring(0, 500));
        throw innerError;
      }
    }
