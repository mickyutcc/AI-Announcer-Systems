...
def generate_luk_thung():
    ...
    try:
        headers = {"Cookie": SUNO_COOKIE, "Content-Type": "application/json"}
        ...
        if response.status_code == 200:
            ...
            print("\n💡 Troubleshooting:")
            print("1. Check if SUNO_COOKIE in .env matches suno-api/.env")
            print("2. Make sure Suno Server is running with the same cookie")
            print("3. Try restarting Suno Server")
            return None
        else:
            print("\n❌ Generation failed!")
            print(f"Response: {response.text}")
